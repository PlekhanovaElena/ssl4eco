...
Please download original dataset from [here](https://huggingface.co/datasets/nascetti-a/BioMassters). The original data contain all Sentinel-2 bands except 1, 9 and 10. We only drop band 10, therefore band 1 and 9 need to be added manually as zero matrices along with the NDVI-band through the [pre-proccessing script](downstream_tasks/add_ndvi_band_BIOMASSTERS.py). From this point on, the [dataset code](datasets/biomassters_dataset.py) automatically takes care of parsing the images and turning the pixel-wise AGBM labels into a 1-D label vector using binning.
