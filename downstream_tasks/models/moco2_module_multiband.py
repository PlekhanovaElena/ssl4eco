...
from pl_bolts.metrics.aggregation import precision_at_k


class MocoV2(LightningModule):

    def __init__(self, arch, moco_dim, moco_k, bands, emb_spaces=3, *args, **kwargs):
        super().__init__()
        ...
        # create the encoders
        template_model = getattr(torchvision.models, arch)
        self.encoder_q = template_model(num_classes=self.hparams.moco_dim)
        self.encoder_k = template_model(num_classes=self.hparams.moco_dim)

        if bands=='B9':
            self.encoder_q.conv1 = torch.nn.Conv2d(9,64,kernel_size=(7,7),stride=(2,2),padding=(3,3),bias=False)
            self.encoder_k.conv1 = torch.nn.Conv2d(9,64,kernel_size=(7,7),stride=(2,2),padding=(3,3),bias=False)
        elif bands=='B12':
        ...
        self.mlp_dim = 512 * (1 if arch in ['resnet18', 'resnet34'] else 4)
        self.heads_q = nn.ModuleList([
            nn.Sequential(nn.Linear(self.mlp_dim, self.mlp_dim), nn.ReLU(), nn.Linear(self.mlp_dim, moco_dim))
        ...
        # create the queue
        self.register_buffer("queue", torch.randn(emb_spaces, moco_dim, moco_k))
    ...
    def _momentum_update_key_encoder(self):
        """
        arch update of the key encoder
        """
        for param_q, param_k in zip(self.encoder_q.parameters(), self.encoder_k.parameters()):
            em = self.hparams.moco_m
    ...
    def _dequeue_and_enqueue(self, keys, queue_idx):
        ...
        assert self.hparams.moco_k % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue[queue_idx, :, ptr:ptr + batch_size] = keys.T
        ptr = (ptr + batch_size) % self.hparams.moco_k  # move pointer
    ...
    def forward(self, img_q, img_k):
        ...
        for i in range(self.hparams.emb_spaces):
            ...
            l = torch.cat([l_pos, l_neg], dim=1)  # logits: Nx(1+moco_k)
            l /= self.hparams.moco_t  # apply temperature
            logits.append(l)
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.SGD(params, self.hparams.lr,
                              momentum=self.hparams.momentum,
                              weight_decay=self.hparams.weight_decay)
        if self.hparams.cos:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = self.hparams.epochs)
            print("Set SGD optimizer with cosine scheduler")
        else:
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones = self.hparams.schedule, gamma=0.1)
            print("Set SGD optimizer with linear rate scheduler")
...
@torch.no_grad()
def batch_shuffle_ddp(x, device='cuda'):  # pragma: no-cover
    """
    ...
    *** Only support DistributedDataParallel (DDP) model. ***
    ...
    idx_shuffle = torch.randperm(batch_size_all, device=device)
